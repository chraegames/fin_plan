...
export function buildingSpec(zone: number, density: number, wealth: number, level: number, variant: number, size = 1): BuildingSpec {
  ...
  if (zone === ZONE.R) {
    if (density === 1) {
      ...
      height = h + 0.22;
      // a chimney on most houses, a porch or garage on the richer ones
      if (v & 4) parts.push({ kind: 'cyl', cx: x1 - 0.1, cz: z0 + 0.12, r: 0.03, h0: h, h1: h + 0.3, color: trim });
      if (wealth >= 2 && v & 8) box(x0 + 0.05, z1, x0 + 0.4, Math.min(1, z1 + 0.1), 0, floorH * 0.6, wall, trim, false);
      if (wealth === 3 && !(v & 8)) box(x1, z0 + 0.1, Math.min(1, x1 + 0.08), z1 - 0.1, 0, floorH * 0.8, wall, roof, false);
    } else if (density === 2) {
      box(x0, z0, x1, z1, 0, height);
      box(x0 - 0.01, z0 - 0.01, x1 + 0.01, z1 + 0.01, height, height + 0.06, roof, roof, false);
      height += 0.06;
      // stairwell or water tank on the roof
      if (v & 4) box(x0 + 0.1, z0 + 0.1, x0 + 0.3, z0 + 0.3, height, height + 0.12, wall, roof, false);
      else parts.push({ kind: 'cyl', cx: x1 - 0.15, cz: z1 - 0.15, r: 0.06, h0: height, h1: height + 0.14, color: trim });
      if (wealth >= 2) box(x0 - 0.02, z1 - 0.02, x1 + 0.02, z1 + 0.03, floorH * 0.95, floorH * 1.02, trim, trim, false); // balcony ledge
      height += 0.14;
    } else {
      ...
      if (wealth === 3 || v & 4) {
        // crown / antenna
        box(0.5 - 0.02, 0.5 - 0.02, 0.5 + 0.02, 0.5 + 0.02, height, height + 0.3, trim, trim, false);
        height += 0.3;
      } else {
        box(i2 + 0.05, i2 + 0.05, i2 + 0.25, i2 + 0.25, height, height + 0.1, trim, trim, false);
        height += 0.1;
      }
    }
  } else if (zone === ZONE.C) {
    if (density === 1) {
      ...
      // rooftop units and a sign band in the shop's colour
      if (v & 4) box(x0 + 0.08, z0 + 0.08, x0 + 0.22, z0 + 0.2, height, height + 0.08, [0.6, 0.6, 0.62], [0.6, 0.6, 0.62], false);
      const sign = SIGNS[v % 3];
      box(x0 - 0.02, z1 - 0.01, x1 + 0.02, z1 + 0.04, floorH * 0.75, floorH * 0.95, sign, sign, false);
      height += 0.12;
    } else {
      ...
      if (wealth === 3) {
        ...
        // helipad / plant room
        box(0.3, 0.3, 0.7, 0.7, height, height + 0.08, [0.4, 0.42, 0.45], [0.4, 0.42, 0.45], false);
        height += 0.08;
      }
    }
  } else {
    ...
    if (density === 1) {
      ...
      // a storage tank beside dirty and manufacturing sheds, a glass lab block on high-tech
      if (wealth < 3 && v & 4) parts.push({ kind: 'cyl', cx: x0 + 0.12, cz: 0.9, r: 0.08, h0: 0, h1: shedH * 0.7, color: [0.62, 0.64, 0.66] });
      if (wealth === 3) box(x0 + 0.1, 0.82, x1 - 0.1, 0.96, 0, shedH * 0.6, [0.55, 0.7, 0.85], roof);
